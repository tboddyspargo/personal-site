define(['scripts/app'],
  (ngApp) => {
    function homeController($scope, $sce, DataService, $timeout, UtilityService) {
      $scope.name = 'home';
      $scope.progress = 0;
      ...
      Promise.all([proj, edu, emp]).then((data) => {
        $timeout(() => {
          $scope.short_desc = UtilityService.shuffleArray($scope.short_desc);
        });
      });
    };

    ngApp.controller('HomeCtrl', ['$scope', '$sce', 'DataService', '$timeout', 'UtilityService', homeController]);
  });
